...
var Sluggin = function() {
    ...
    function convert(text) {
        return function(search, replace, subject, count) {
            ...
            return search ? s : s[0];
        }(characters, latin, text).toLowerCase().replace(/^\s+|\s+$/g, "").replace(/[_|\s]+/g, "-").replace(/[^a-z\u0400-\u04FF0-9-]+/g, "").replace(/[-]+/g, "-").replace(/^-+|-+$/g, "").replace(/[-]+/g, "-");
    }
    return function(text) {
        var output;
        if ("string" == typeof text) return output = convert(text);
        if ("[object Array]" !== Object.prototype.toString.call(text)) return "";
        for (var values = [], i = 0; i < text.length; i++) output = convert(text[i]), 
        values.push(output);
        return values;
    };
}();
